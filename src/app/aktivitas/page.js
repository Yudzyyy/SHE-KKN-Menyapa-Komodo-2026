...
export default function AktivitasPage() {
  ...
  useEffect(() => {
    const CACHE_KEY = 'she_activities_v2';
    ...
    if (stored) {
      try {
        ...
        console.error("Gagal parsing she_activities_v2 dari localStorage", e);
      }
    }
    ...
    const fetchActivities = async () => {
      ...
      try {
        const { data, error } = await supabase
          .from('aktivitas')
          .select('id, posko, kategori, deskripsi, status, tanggal')
        ...
        if (!hasStoredData) {
          ...
          if (storedFallback) {
            try {
              ...
              setActivities(parsed.data || INITIAL_ACTIVITIES);
            } catch (e) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data && data[0]) {
        ...
        localStorage.setItem('she_activities_v2', JSON.stringify({
          data: updatedList,
          timestamp: Date.now()
        }));
