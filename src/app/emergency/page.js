...
export default function EmergencyPage() {
  ...
  useEffect(() => {
    const CACHE_KEY = 'she_contacts_v2';
    const TTL = 24 * 60 * 60 * 1000; // 24 jam dalam milidetik
    ...
    if (cached) {
      try {
        ...
        console.error("Gagal parsing cache kontak:", e);
      }
    }

    const fetchContacts = async () => {
      // Hanya tampilkan skeleton jika cache tidak valid/kosong
      if (!hasValidCache) {
        setIsLoading(true);
      }
      try {
        const { data, error } = await supabase
          .from('kontak_darurat')
          .select('id, name, type, phone, region, category, description')
        ...
        if (data && data.length > 0) {
          ...
          // Simpan dengan format data + timestamp
        ...
        if (!hasValidCache) {
          ...
          if (storedFallback) {
            try {
              ...
              setContacts(parsed.data || CONTACTS_DATA);
            } catch (e) {
              setContacts(CONTACTS_DATA);
            }
          } else {
