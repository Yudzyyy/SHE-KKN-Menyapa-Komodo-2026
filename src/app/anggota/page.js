    ...
    if (stored) {
      try {
        ...
        if (parsed && parsed.data && parsed.data.length > 0) {
          ...
          if (age < TTL) {
            setIsLoading(false); // Sembunyikan loading skeleton jika cache masih valid
            hasStoredData = true;
          }
        }
      } catch (e) {
        console.error("Gagal parsing she_members_v2 dari localStorage", e);
      }
    }

    const fetchMembers = async () => {
      // Hanya tampilkan skeleton jika tidak ada data sama sekali di cache
      if (!hasStoredData) {
        setIsLoading(true);
      }
      try {
        ...
        if (data && data.length > 0) {
          ...
          localStorage.setItem(CACHE_KEY, JSON.stringify({
            data: cleanMembersForCache(mappedData),
            timestamp: Date.now()
          }));
        } else {
          ...
          if (insertError) {
            ...
            localStorage.setItem(CACHE_KEY, JSON.stringify({
              data: cleanMembersForCache(mappedInserted),
              timestamp: Date.now()
            }));
          }
        }
        ...
        if (!hasStoredData) {
          ...
          if (storedFallback) {
            try {
              ...
              setMembers(parsed.data || INITIAL_MEMBERS);
            } catch (e) {
              setMembers(INITIAL_MEMBERS);
            }
          } else {
