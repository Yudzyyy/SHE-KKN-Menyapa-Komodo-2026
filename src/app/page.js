...
export default async function Home() {
  ...
  try {
    // Fetch anggota dan insiden secara paralel agar lebih cepat
    const [
      { data: members, error: mErr },
      { data: incidents, error: iErr }
    ] = await Promise.all([
      supabase.from('anggota').select('*'),
      supabase.from('insiden').select('*').order('tanggal', { ascending: false })
    ]);
    ...
    // Selalu update recentIncidents dari Supabase (kosong jika tidak ada data)
    if (incidents) {
      stats.recentIncidents = incidents.slice(0, 3).map((inc, i) => {
