...
const cleanMembersForCache = (list) => {
  if (!list) return [];
  return list.map(m => ({
    id: m.id,
    name: m.name || m.nama,
    ...
    major: m.major,
    phone: m.phone,
    profilePic: (m.profilePic && m.profilePic.startsWith('data:')) ? `https://i.pravatar.cc/150?u=${m.id}` : m.profilePic,
    canSwim: m.canSwim,
...
export default function KesehatanPage() {
  ...
  useEffect(() => {
    const CACHE_KEY = 'she_members_v2';
    const TTL = 60 * 60 * 1000; // 1 jam dalam milidetik

    // 1. Load dari LocalStorage terlebih dahulu agar UI instant
    ...
    if (stored) {
      try {
        ...
        if (parsed && parsed.data && parsed.data.length > 0) {
          const mappedData = parsed.data.map(m => {
            let extra = {};
            if (m.catatan) {
              try {
                extra = typeof m.catatan === 'string' ? JSON.parse(m.catatan) : m.catatan;
              } catch (e) {
            ...
            return {
              id: m.id,
              name: m.name || m.nama,
              posko: m.village || m.posko || "Warloka Pesisir",
              kondisi: m.kondisi || "Sehat",
              medicalHistory: extra.medicalHistory || m.medicalHistory || "Tidak ada riwayat",
              medicalNotes: extra.medicalNotes || m.medicalNotes || "Sehat walafiat",
              profilePic: m.profilePic || extra.profilePic || `https://i.pravatar.cc/150?u=${m.id}`,
        ...
        console.error("Gagal load cache she_members_v2 di kesehatan", e);
      }
    }
    ...
    const fetchMembers = async () => {
      ...
      try {
        const { data, error } = await supabase
          .from('anggota')
          .select('id, nama, posko, kondisi, catatan, jabatan, no_hp')
        ...
        if (data && data.length > 0) {
          ...
          // Juga simpan data asli ke she_members_v2 untuk sinkronisasi dengan halaman anggota
          const mappedForMembersPage = data.map(m => {
            let extra = {};
            if (m.catatan) {
              try { extra = JSON.parse(m.catatan); } catch(e) {}
            }
            return {
              id: m.id,
              name: m.nama,
          ...
          localStorage.setItem(CACHE_KEY, JSON.stringify({
            data: cleanMembersForCache(mappedForMembersPage),
            timestamp: Date.now()
          }));
        }
      } catch (err) {
  ...
  const handleUpdateSubmit = async (e) => {
    ...
    try {
      ...
      // Sync ke cache localStorage
      const cached = localStorage.getItem('she_members_v2');
      if (cached) {
        try {
          const parsed = JSON.parse(cached);
          if (parsed && parsed.data) {
            const updatedCache = parsed.data.map(m => m.id === selectedMember.id ? {
              ...m,
              kondisi: formKondisi,
              medicalNotes: formMedicalNotes,
              catatan: JSON.stringify(extra)
            ...
            localStorage.setItem('she_members_v2', JSON.stringify({
              data: cleanMembersForCache(updatedCache),
              timestamp: Date.now()
            }));
          }
        } catch(e) {
          console.error("Gagal sinkronisasi cache saat update kesehatan:", e);
        }
      }
